...
<html lang="ua">
	...
	<body>
		<div id="login">
			...
			<? if ($factory) : ?>
			<h3 class="text-center text-white font-ubuntu"><?=$factory['factory_name']?></h3>
			<? endif ?>
			<div class="container">
					...
					<div id="login-column" class="col-md-6">
						<div class="login-box col-md-12">
							<?=form_open('', array('id' => 'login-form', 'class' => 'form')); ?>
								...
								<? if ($factory) : ?>
								<input type="text" name="factory" id="factory" value="<?=$factory['factory_id']?>" class="d-none">
								<? else : ?>
								<div class="form-group">
									<label for="factory" class="text-blue">Підприємство:</label><br>
									<select name="factory" id="factory" class="form-control">
										<option value="">оберіть зі списку</option>
										<? foreach ($factories as $row) : ?>
										<option value="<?=$row['factory_id']?>" <?=(set_value('factory') == $row['factory_id']) ? 'selected' : ''?>><?=$row['factory_name']?></option>
										<? endforeach ?>
									</select>
									<small class="text-danger"><?=form_error('factory'); ?></small>
								</div>
								<? endif ?>
								<div class="form-group">
								...
								<div class="form-group pt-3 text-center">
									<input type="submit" name="submit" class="btn btn-blue btn-md" value="Увійти до разрахункового центру">
								</div>
							<?=form_close(); ?>
						</div>
					</div>
